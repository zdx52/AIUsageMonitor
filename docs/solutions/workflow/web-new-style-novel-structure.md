...
### 补充（2026-08-15）：characters.md 分组格式兼容

新书根目录**同时存在**平铺 `characters.md` 时（八零绣娘：人物册在根目录，
角色卡也在 `设定/角色/`），fallback 不触发，走的是 `parse_characters()`，
而它只认旧格式（`## 女主：林小满` 每个 `##` = 一个角色）。新书人物册是
**分组格式**：
...
## 主要配角
### 赵桂英（娘）
## 角色说话方式速查（写作时对照）   ← 无 ### 子标题，应跳过
```

旧解析器症状：分组标题被当成角色名（"主角（核心人物）"/"主要配角"），
且内容行宽匹配 `"主" in val` 把"主要配角"误判为**主角** → 主角分组下
出现"主要配角"卡片。

修复（`parse_characters`）：
- 检测文本含 `### ` 行 → 自动切分组模式（`##` = 角色组推断 role，
  `###` = 角色卡片）；无 `###` 子标题的组（角色说话方式速查）自动跳过
- `"主" in val` → `("主角" in val and "配角" not in val)`，避免
  "主要配角"/"主角团"误判

回归：八零绣娘（分组格式：姜绣→主角，10 配角✓）+ 七零年代（旧格式：
林巧→主角，15 配角✓）双验证通过。
...
- characters.md 解析前先看格式：含 `### ` 二级标题 = 分组格式，别用旧解析器
- 角色 role 判定永远用精确词（"主角"且不含"配角"），禁止单字宽匹配（"主"）
- 新书既有平铺 characters.md 又有 设定/角色/ 目录时，先修平铺解析，
  不要只依赖 fallback（fallback 不会触发）
